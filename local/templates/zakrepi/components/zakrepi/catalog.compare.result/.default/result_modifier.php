...
foreach($arResult["ITEMS"] as $index=>$arItem)
{
	...
	if (count($arFile) > 0 && $arFile["HEIGHT"] > 130){
		...
		$file = CFile::ResizeImageGet($arFile, array('width'=>$width, 'height'=>130), BX_RESIZE_IMAGE_PROPORTIONAL, true);
		$arResult["ITEMS"][$index]["DISPLAY_IMG_SRC"] = $file["src"];
	}
	else
		$arResult["ITEMS"][$index]["DISPLAY_IMG_SRC"] = $arFile["SRC"];
}
?>