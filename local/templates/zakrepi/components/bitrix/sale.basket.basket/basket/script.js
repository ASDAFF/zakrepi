...
function updateBasketTable(basketItemId, res)
{
		...
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			...
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('current_price_' + id))
					BX('current_price_' + id).innerHTML = priceShow(item.FULL_PRICE);
	...
	if (!!res.BASKET_DATA.COUPON)
		$("#coupon").data("coupon", res.BASKET_DATA.COUPON);
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_FORMATED'))
			BX('allSum_FORMATED').innerHTML = priceShow(res['BASKET_DATA']['allSum']);

		if (BX('PRICE_WITHOUT_DISCOUNT')){
			var price_without_discount = parseFloat(res['BASKET_DATA']['allSum']) + parseFloat(res['BASKET_DATA']['DISCOUNT_PRICE_ALL']);
			BX('PRICE_WITHOUT_DISCOUNT').innerHTML = priceShow(price_without_discount);
		}

		if (BX('DISCOUNT_PRICE_ALL'))
			BX('DISCOUNT_PRICE_ALL').innerHTML = priceShow(res['BASKET_DATA']['DISCOUNT_PRICE_ALL']);

		if (res['BASKET_DATA']['allSum'] < ORDER_MIN_SUMM){
			$("#free_delivery_note").hide();
			$("#min_summ_note").show();
			$("#orderBtn").attr("disabled", "disabled");
		}
		else{
			$("#free_delivery_note").show();
			$("#min_summ_note").hide();
			$("#orderBtn").removeAttr("disabled");
			if (res['BASKET_DATA']['allSum'] < FREE_DELIVERY_SUMM){
				$("#free_delivery_need_summ").show();
				$("#summ_with_delivery").hide();
				$("#summ_without_delivery").show();
				var needSumm = priceShow(FREE_DELIVERY_SUMM - res['BASKET_DATA']['allSum']);
				$("#free_delivery_need_summ").find(".primary-text").html(needSumm);				
			}
			else{
				$("#summ_with_delivery").show();
				$("#free_delivery_need_summ").hide();
				$("#summ_without_delivery").hide();
			}
		}
...
function enterCoupon()
{
	...
	else if($("#coupon").data('coupon').length > 0)
		recalcBasketAjax({'delete_coupon' : $("#coupon").data('coupon')});
}
...
function updateQuantity(controlId, basketId, ratio, bUseFloatQuantity)
{
	...
	minVal = 1;
	maxVal = $("#QUANTITY_INPUT_"+basketId).attr("max");

	if (newVal == minVal)
		$("#basket_items").find("#"+basketId).find(".btn-down").attr("disabled", "disabled");
	else
		$("#basket_items").find("#"+basketId).find(".btn-down").removeAttr("disabled");

	if (newVal == maxVal)
		$("#basket_items").find("#"+basketId).find(".btn-up").attr("disabled", "disabled");
	else
		$("#basket_items").find("#"+basketId).find(".btn-up").removeAttr("disabled");
...
function recalcBasketAjax(params)
{
	...
	var property_values = {},
		action_var = BX('action_var').value,
		items = $('#basket_items'),
		basketItems = items.find(".basket-item"),
	...
	{
		basketItems.each(function(indx, element){
			var id = $(element).attr("id");
			postData['QUANTITY_' + id] = BX('QUANTITY_' + id).value;
		});

		//for (i = 1; itemsCount > i; i++)
...
function clearBasketArea(){
	$("#basket_form_container").html('<div class="page-empty"><div class="title large-text color-text text-light light center-align">Ваша корзина пуста</div></div>');
}
	...
	BX.ajax({
		...
		onsuccess: function(result)
		{
			if (result["CODE"] == "SUCCESS"){
				...
				var itemsCount = $('#basket_items').find(".basket-item").length;
				if (itemsCount == 1)
				{
					//очистка всей области корзины
					clearBasketArea();
