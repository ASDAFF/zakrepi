...
while($arPerson = $dbPerson->GetNext())
{
	...
	if($bProp)
	{
		...
		if (count($arUsersGroups) > 0){
			$arComponentParameters["PARAMETERS"]["GROUP_".$arPerson["ID"]] =  array(
				"NAME" => GetMessage("SOA_USER_GROUP")." \"".$arPerson["NAME"]."\" (".$arPerson["LID"].")".GetMessage("SOA_USER_GROUP_2"),
				"TYPE"=>"LIST", 
				"MULTIPLE"=>"Y",
		...
		$arComponentParameters["PARAMETERS"]["CONTACT_PROPERTIES_".$arPerson["ID"]] =  array(
			"NAME" => GetMessage("SOA_CONTACT_PROPERTIES_GROUP")." \"".$arPerson["NAME"]."\" (".$arPerson["LID"].")",
			"TYPE"=>"LIST", 
			"MULTIPLE"=>"Y",
			"VALUES" => $arPropsGroup[$arPerson["ID"]],
			"DEFAULT"=>"",
			"COLS"=>25,
			"ADDITIONAL_VALUES"=>"N",
			"PARENT" => "BASE",
		...
		$arComponentParameters["PARAMETERS"]["DELIVERY_PROPERTIES_".$arPerson["ID"]] =  array(
			"NAME" => GetMessage("SOA_DELIVERY_PROPERTIES_GROUP")." \"".$arPerson["NAME"]."\" (".$arPerson["LID"].")",
			"TYPE"=>"LIST", 
			"MULTIPLE"=>"Y",
			"VALUES" => $arPropsGroup[$arPerson["ID"]],
			"DEFAULT"=>"",
...
$arComponentParameters["PARAMETERS"]["DEFAULT_PERSON_TYPE_ID"] =  array(
	"NAME" => GetMessage("SOA_DEFAULT_PERSON_TYPE_ID"),
	"TYPE"=>"LIST", 
	"MULTIPLE"=>"N",
	"VALUES" => $arPersonType,
	"DEFAULT"=>"",
...
$dbPaySystem = CSalePaySystem::GetList(
	...
	array("ACTIVE" => "Y", "PSA_HAVE_PAYMENT" => "Y")
);
while ($arPaySystem = $dbPaySystem->Fetch())
{
	$arPayment[$arPaySystem["ID"]] = $arPaySystem["NAME"].' ['.$arPaySystem["ID"].']';
}

$arComponentParameters["PARAMETERS"]["PAYMENT_GROUP_1"] =  array(
	"NAME" => GetMessage("SOA_PAYMENT_GROUP_1"),
	"TYPE"=>"LIST", 
	"MULTIPLE"=>"Y",
	"VALUES" => $arPayment,
	"DEFAULT"=>"",
...
$arComponentParameters["PARAMETERS"]["PAYMENT_GROUP_2"] =  array(
	"NAME" => GetMessage("SOA_PAYMENT_GROUP_2"),
	"TYPE"=>"LIST", 
	"MULTIPLE"=>"Y",
	"VALUES" => $arPayment,
	"DEFAULT"=>"",
