		...
		if (!empty($arSource))
		{
			foreach ($arSource as $arProperties)
			{
				...
				elseif ($arProperties["TYPE"] == "TEXT")
				{
					...
					<div class="field">
						<input type="<?=$type?>" maxlength="250" size="<?=$arProperties["SIZE1"]?>" <?if ($checkEmail):?>onchange="checkUserEmail(this);"<?endif;?> value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" class="PROP_INPUT_<?=$arProperties["CODE"]?>" id="<?=$arProperties["FIELD_NAME"]?>" <?/*if ($arProperties["REQUIED"] == "Y"):?>required<?endif;*/?> />
					</div>
					<?
				}
