			...
			foreach ($arDelivery["PROFILES"] as $profile_id => $arProfile)
			{
				...
				<p>
					...
					<label class="radio-lbl btn-toggle-block" for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>">
						...
						<span style="display:none" onclick="BX('ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>').checked=true;submitForm();">
		...
		else // stores and courier
		{
			...
			<p>
				...
				<label class="radio-lbl btn-toggle-block" for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?=$clickHandler?>>
					<?= htmlspecialcharsbx($arDelivery["NAME"])?> <span class="color-text text-light"><?= htmlspecialcharsbx($arDelivery["DESCRIPTION"])?></span>
					<?/*if (count($arDelivery["STORE"]) > 0):?>
						<span class="color-text text-light" id="select_store"<?if(strlen($arResult["STORE_LIST"][$arResult["BUYER_STORE"]]["TITLE"]) <= 0) echo " style=\"display:none;\"";?>>
