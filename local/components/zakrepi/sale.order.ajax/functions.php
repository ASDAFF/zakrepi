...
function getFormatedProperties($personTypeId, &$arResult, &$arUserResult, &$params)
{
	...
	while ($arProperties = $dbProperties->GetNext())
	{
		...
		if ($arProperties["CODE"] == "NAME" && !empty($arUser["NAME"]))
			$arResult["ORDER_PROP"]["USER_PROPS_".$flag][$arProperties["ID"]]["VALUE"] = $arUser["NAME"];
		
		if ($arProperties["CODE"] == "LASTNAME" && !empty($arUser["LAST_NAME"]))
			$arResult["ORDER_PROP"]["USER_PROPS_".$flag][$arProperties["ID"]]["VALUE"] = $arUser["LAST_NAME"];
		
		if ($arProperties["CODE"] == "PHONE" && !empty($arUser["WORK_PHONE"]))
			$arResult["ORDER_PROP"]["USER_PROPS_".$flag][$arProperties["ID"]]["VALUE"] = $arUser["WORK_PHONE"];
		
		if ($arProperties["CODE"] == "EMAIL" && !empty($arUser["EMAIL"]))
			$arResult["ORDER_PROP"]["USER_PROPS_".$flag][$arProperties["ID"]]["VALUE"] = $arUser["EMAIL"];
		
		if ($arProperties["CODE"] == "LOCATION")
			$arResult["LOCATION_PROP_ID"] = $arProperties["ID"];
