...
$arResult = array(
		"DEFAULT_PERSON_TYPE_ID" => $arParams["DEFAULT_PERSON_TYPE_ID"],
		"PERSON_TYPE" => array(),
		"PAY_SYSTEM" => array(),
		"ORDER_PROP" => array(),
		"DELIVERY" => array(),
...
if($arParams["ALLOW_AUTO_REGISTER"] == "Y" && $arResult["AUTH"]["new_user_registration"] == "N")
	$arParams["ALLOW_AUTO_REGISTER"] = "N";
	...
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if (!$isOrderPlaced)
		{
			...
			foreach($arResult["ORDER_PROP"]["USER_PROPS_Y"] as $propID=>$arProp){
				if ($arProp["IS_LOCATION"] == "Y"){
					foreach($arProp["VARIANTS"] as $propVariantID=>$arPropVariant){
						if ($arPropVariant["ID"] == $LOCATION_ID){
							$arResult["ORDER_PROP"]["USER_PROPS_Y"][$propID]["VARIANTS"][$propVariantID]["SELECTED"] = "Y";
						}
					}
				}
			}
			$arUserResult["DELIVERY_LOCATION"] = $LOCATION_ID;
		}
		...
		if (count($arResult["ORDER_PROP"]["RELATED"]) == 0)
		{
			...
			if (isset($arRelFilter["RELATED"]) && count($arRelFilter["RELATED"]) > 0)
			{
				...
				foreach($arRes as $key=>$arResProp){
					$arResult["ORDER_PROP"]["RELATED"][$arResProp["CODE"]] = $arResProp;
					if (empty($arResProp["VALUE"]) && empty($_POST["IS_ADDRESS_CHANGE"]) && empty($_POST["IS_ADDRESS_NEW"])){
						$arResult["ORDER_PROP"]["RELATED"][$arResProp["CODE"]]["VALUE"] = $arResult["JS_USER_ADDRESS"][$arResult["SELECTED_USER_ADDRESS"]][$arResProp["CODE"]];
					}
				}
		...
		else
		{
			...
			$arResult["ORDER_TOTAL_PRICE_NOT_FORMATED"] = $orderTotalSum;
			/* Tax End */
	...
	else
	{
		...
		if($arOrder)
		{
			...
			while ($arVals = $db_vals->Fetch()){
				$arResult["ORDER_INFO"]["ORDER_PROPS"][$arVals["CODE"]] = $arVals["VALUE"];
				if ($arVals["CODE"] == "LOCATION"){
					$arLocs = CSaleLocation::GetByID($arVals["VALUE"], LANGUAGE_ID);
					$arResult["ORDER_INFO"]["ORDER_PROPS"][$arVals["CODE"]] = $arLocs["CITY_NAME"];
				}
			}
